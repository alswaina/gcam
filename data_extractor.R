...
#
#.MAIN_QUERY file to read the query from. Path uses forward-slash as seperator i.e. "/"
main.query <- "/Users/fahad/Desktop/GCAM/gcam-v5.4-Mac-Release-Package/output/queries/Main_queries.xml"
#
#.QUERY.BY = {"title", "xml} 
...
process_queries <- function(myconn, db.path, main.query, queries_xml, scenario){
  ...
  for(query.counter in names(queries_xml)){
    ...
    output.dirname <- dirname(db.path)
...
process_query <- function(myconn, scenario, output.filename, output.dirname, query.by, query.xml, query.query, query.title, query.counter){
  ...
  if(!is.null(query)){
    ...
    write_output(result = result, output.filename = output.filename, output.dirname = output.dirname)
  }else{
...
#output
write_output <- function(result, output.filename, output.dirname){
  #<db_path>/<output.filename>_<db_name>
  
  if(!is.null(result)){
    output.filepath <- paste0(output.dirname,"/", output.filename)
